...
const ClosEdit = () => {
  ...
  const [idstudyprogram, setIdstudyprogram] = useState("");
  const [idperiod, setIdperiod] = useState("");
  const [code, setCode] = useState("");
  const [precentage, setPrecentage] = useState("");
  const [description, setDescription] = useState("");
  const [rubrikasi, setRubrikasi] = useState("");
  const [pembimbingChecked, setPembimbingChecked] = useState(false);
  const [pengujiChecked, setPengujiChecked] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setIdstudyprogram(idstudyprogram);
        setIdperiod(idperiod);
        setCode(code);
        setPrecentage(precentage);
        setDescription(description);
        setRubrikasi(rubrikasi);
      } catch (err) {
        console.log(err);
        navigate("/home");
      } finally {
                ...
                <div className="col-sm-6">
                  <div className="card">
                    ...
                    <div className="card-body">
                      ...
                      {cLO === null ? (
                        <div>
                          <label className="checkbox-inline pr-3">
                            Pembimbing:
                            <input
                              type="checkbox"
                              name="pembimbing"
                              checked={pembimbingChecked}
                            />
                          </label>
